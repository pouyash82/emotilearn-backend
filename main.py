...
# ══════════════════════════════════════════════════════════════════════════
# MULTI-FACE TRACKING (runs alongside original session)
# ══════════════════════════════════════════════════════════════════════════
import math

multi_session = {
    "start_time": None, "faces": {},
    "next_face_id": 1, "known_positions": [],
}
...
def get_or_assign_face_id(bbox):
    ...
    multi_session["known_positions"].append({
        "id": face_id, "last_bbox": bbox,
        "last_center": (cx, cy), "last_seen": now,
    })
    return face_id
...
def get_face_session(face_id):
    if face_id not in multi_session["faces"]:
        multi_session["faces"][face_id] = {
            ...
            "engagement": [], "prev_emotion": None, "streak": 0,
            "first_seen": datetime.now().isoformat(),
            "last_seen": datetime.now().isoformat(),
        }
    return multi_session["faces"][face_id]

def log_emotion_for_face(face_id, emotion, scores):
    if multi_session["start_time"] is None:
        reset_multi_session()
    fd = get_face_session(face_id)
    confidence = scores.get(emotion, 0.5)
    fd["emotions"].append({
        "time": datetime.now().isoformat(), "emotion": emotion,
        "scores": scores, "confidence": round(confidence, 4), "face_id": face_id,
    })
    fd["dominant"][emotion] = fd["dominant"].get(emotion, 0) + 1
    fd["last_seen"] = datetime.now().isoformat()
    base_map = {"happiness":0.85,"surprise":0.80,"anger":0.65,"fear":0.55,"disgust":0.50,"sadness":0.35,"neutral":0.30}
    base = base_map.get(emotion, 0.5)
    conf_boost = max(-0.1, min(0.15, (confidence - 0.5) * 0.3))
    variety = 0.1 if (fd["prev_emotion"] and fd["prev_emotion"] != emotion) else 0.0
    if variety > 0:
        fd["streak"] = 0
    else:
        fd["streak"] += 1
    streak_pen = min(0.20, max(0, (fd["streak"] - 10) * 0.01))
    express = max(-0.1, min(0.15, (0.5 - scores.get("neutral", 0.5)) * 0.2))
    eng = max(0.05, min(1.0, base + conf_boost + variety - streak_pen + express))
    eng = round(eng, 4)
    fd["prev_emotion"] = emotion
    fd["engagement"].append({"time": fd["last_seen"], "score": eng, "emotion": emotion})
    return eng

def get_face_profile(face_id):
    if face_id not in multi_session["faces"]:
        return {"error": f"No data for {face_id}"}
    fd = multi_session["faces"][face_id]
    if not fd["emotions"]:
    ...
    avg = round(sum(e["score"] for e in recent)/len(recent)*100,1) if recent else 0
    overall = round(sum(e["score"] for e in fd["engagement"])/len(fd["engagement"])*100,1)
    lbl = "High" if avg>=65 else "Medium" if avg>=40 else "Low"
    uq = len([e for e,c in fd["dominant"].items() if c>0])
    return {"face_id": face_id, "first_seen": fd["first_seen"], "last_seen": fd["last_seen"],
...
def get_class_profile():
    ...
    profiles = []; total_det = 0; all_eng = []
    for fid in multi_session["faces"]:
        p = get_face_profile(fid)
        if "error" not in p:
            profiles.append(p); total_det += p["total_detections"]; all_eng.append(p["avg_engagement"])
    ...
    return {"session_start": multi_session["start_time"], "total_faces_detected": len(multi_session["faces"]),
        "total_detections": total_det, "avg_class_engagement": avg_cls, "engagement_label": lbl,
        "face_profiles": profiles, "most_engaged": ranked[0]["face_id"] if ranked else None,
...
def detect_and_predict(frame_bgr):
    try:
        ...
        return frame_bgr, []
    results = []
    colors = [(0,255,0),(255,165,0),(0,191,255),(255,0,255),(255,255,0),(0,255,255),(255,105,180),(50,205,50)]
    for idx, (x, y, w, h) in enumerate(faces):
        try:
            ...
            face_id = get_or_assign_face_id([int(x), int(y), int(w), int(h)])
            results.append({
                "face_id"   : face_id,
            ...
            cv2.rectangle(frame_bgr, (x, y-th-10), (x+tw+4, y), color, -1)
...
async def detect_emotion_api(file: UploadFile = File(...)):
    ...
    for r in results:
        log_emotion_for_face(r["face_id"], r["emotion"], r["scores"])
    ...
    # Log emotion for session tracking (original single-face)
...
async def session_end():
    ...
    return {"status": "Session ended", "profile": profile, "class_profile": class_profile}
...
async def detect_emotion_multi(file: UploadFile = File(...)):
    ...
    face_results = []
    for r in results:
        eng = log_emotion_for_face(r["face_id"], r["emotion"], r["scores"])
        fd = get_face_session(r["face_id"])
        face_results.append({
            "face_id": r["face_id"], "bbox": r["bbox"],
            "emotion": r["emotion"],
            "emotions": {k: round(v*100,1) for k,v in r["scores"].items()},
            "confidence": round(r["confidence"]*100, 1),
            "engagement": round(eng*100, 1),
            "total_detections": len(fd["emotions"]),
        })
    if results:
        log_emotion(results[0]["emotion"], results[0]["scores"])
...
async def face_profile_ep(face_id: str):
...
async def multi_engagement_summary():
    ...
    summary = []
    for fid, data in multi_session["faces"].items():
        if data["engagement"]:
            avg = round(sum(e["score"] for e in data["engagement"])/len(data["engagement"])*100,1)
            latest = round(data["engagement"][-1]["score"]*100,1)
        else:
            avg = 0; latest = 0
        summary.append({"face_id": fid, "avg_engagement": avg,
            "current_engagement": latest, "total_detections": len(data["emotions"])})
    return {"face_count": len(summary), "faces": sorted(summary, key=lambda f: f["avg_engagement"], reverse=True)}
...
async def export_multi_csv():
    ...
    try:
        import csv as csv2
        output = io.StringIO()
        writer = csv2.writer(output)
        writer.writerow(["face_id","timestamp","emotion","confidence","anger","disgust","fear","happiness","neutral","sadness","surprise","engagement_score"])
        for fid, fd in multi_session["faces"].items():
            eng_map = {e["time"]: e["score"] for e in fd["engagement"]}
